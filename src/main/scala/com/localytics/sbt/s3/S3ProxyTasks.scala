...
object S3ProxyTasks {
  ...
  def startS3ProxyTask = (downloadS3Proxy, s3ProxyDownloadDir, s3ProxyDownloadFile, s3ProxyPort, s3ProxyDataDir, streams) map {
    case (_, downloadDir, downloadFile, port, dataDir, streamz) =>
      val args = Seq("java") ++
