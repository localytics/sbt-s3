...
object S3ProxyKeys {
  lazy val s3ProxyVersion = settingKey[String]("S3Proxy version to download. Defaults to 1.5.3.")
  ...
  lazy val baseS3ProxySettings = Seq(
    ...
    s3ProxyTestCleanup := {
      ...
      val s3ProxyDataDirValue = s3ProxyDataDir.value
      val s3ProxyCleanAfterStopValue = s3ProxyCleanAfterStop.value
      Tests.Cleanup(() => StopS3Proxy(streamsValue, s3ProxyDataDirValue, s3ProxyCleanAfterStopValue))
    },
